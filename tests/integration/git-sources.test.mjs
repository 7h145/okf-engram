...
test("M2b captures clean committed bytes with immutable Git identity and a selector", async (t) => {
  ...
  const result = parse(await run([
    "capture-source", "project:docs/guide ü.md", "--to", raw,
    "--selector-kind", "heading", "--selector-value", "Details", "--region-to", region,
...
test("M2b dirty, untracked, filtered, non-Git, and no-Git-command captures remain digest-only", async (t) => {
  ...
  let result = parse(await run([
    "capture-source", "project:docs/guide ü.md", "--to", path.join(dirty.root, "dirty.bin"),
    "--project-root", dirty.root, "--json",
  ]));
  ...
  result = parse(await run([
    "capture-source", "project:docs/new.txt", "--to", path.join(dirty.root, "new.bin"),
    "--project-root", dirty.root, "--json",
  ]));
  ...
  result = parse(await run([
    "capture-source", "project:docs/guide ü.md", "--to", path.join(filtered.root, "filtered.bin"),
    "--project-root", filtered.root, "--json",
  ]));
  assert.equal(result.git.reason, "working-bytes-differ");
  ...
  result = parse(await run([
    "capture-source", "project:source.txt", "--to", path.join(plain, "plain.bin"),
    "--project-root", plain, "--json",
  ]));
  assert.equal(result.git.reason, "not-a-repository");

  result = parse(await run([
    "capture-source", "project:source.txt", "--to", path.join(plain, "nogit.bin"),
    "--project-root", plain, "--json",
  ], { env: { PATH: "" } }));
...
test("M2b external file resources use explicit non-portable repository hints", async (t) => {
  ...
  await putConcept(context, "evidence/external", conceptDraft({
    id: "guide", resource, title: "External guide", digest: capture.digest,
    git: capture.git.identity,
  }));
  const resolved = parse(await run([
    "resolve-source", "evidence/external", "guide", "--to", path.join(projectRoot, "external-resolved.bin"),
...
test("M2b an explicit revision captures commit bytes without checkout", async (t) => {
  ...
  const result = parse(await run([
    "capture-source", "project:docs/guide ü.md", "--ref", oldCommit, "--to", output,
    "--project-root", root, "--json",
  ]));
  assert.equal(result.git.state, "pinned");
  ...
  const unavailable = parse(await run([
    "capture-source", "project:docs/guide ü.md", "--ref", "0".repeat(40),
    "--to", unavailableOutput, "--project-root", root, "--json",
  ]));
  assert.equal(unavailable.state, "unavailable");
  assert.equal(unavailable.git.reason, "ref-unavailable");
...
test("M2b resolves a recorded blob, verifies every identity component, and reports live drift", async (t) => {
  ...
  const checked = parse(await run([
    "check-sources", "evidence/guide", "--project-root", root, "--json",
  ]));
  ...
  const missingLive = parse(await run([
    "resolve-source", "evidence/guide", "guide", "--to", path.join(root, "resolved-missing-live.bin"),
    "--project-root", root, "--json",
  ]));
  ...
  const checkedMissing = parse(await run([
    "check-sources", "evidence/guide", "--project-root", root, "--json",
  ]));
  assert.equal(checkedMissing[0].state, "missing");
  assert.equal(checkedMissing[0].gitState, "verified");
});
...
test("M2b missing repositories, objects, and identity mismatches never produce source bytes", async (t) => {
  ...
  const base = {
    id: "guide", resource: capture.resource, title: "Guide", digest: capture.digest,
    git: capture.git.identity,
  };
  const cases = [
    ["missing-object", { ...base, git: { ...base.git, blob: { ...base.git.blob, oid: "0".repeat(40) } } }, "object-unavailable"],
  ...
  for (const [id, entry, reason] of cases) {
    await putConcept(context, `evidence/${id}`, conceptDraft(entry));
    const output = path.join(root, `${id}.out`);
    const result = parse(await run([
      "resolve-source", `evidence/${id}`, "guide", "--to", output,
      "--project-root", root, "--json",
    ]));
...
test("M2b SHA-256 repositories tag commit and blob algorithms explicitly", async (t) => {
  ...
  await initializeBundle(context);
  const result = parse(await run([
    "capture-source", "project:source.txt", "--to", path.join(root, "captured.bin"),
    "--project-root", root, "--json",
  ]));
  assert.equal(result.git.state, "pinned");
  assert.deepEqual(
    [result.git.identity.commit.algorithm, result.git.identity.blob.algorithm],
    ["sha256", "sha256"],
  );
...
test("M2b shallow clones report unavailable historical objects without fetching", async (t) => {
  ...
  await initializeBundle(context);
  await putConcept(context, "evidence/shallow", conceptDraft({
    id: "guide", resource: "project:docs/guide ü.md", title: "Guide",
    digest: capture.digest, git: capture.git.identity,
  }));
  const output = path.join(clone, "old.out");
  const result = parse(await run([
    "resolve-source", "evidence/shallow", "guide", "--to", output,
    "--project-root", clone, "--json",
  ]));
  assert.equal(result.state, "unavailable");
...
test("M2b LFS pointer blobs are never returned as artifact payloads", async (t) => {
  ...
  const result = parse(await run([
    "capture-source", "project:asset.pdf", "--to", output, "--project-root", root, "--json",
  ]));
  assert.equal(result.state, "unavailable");
...
test("M2b capture rejects a separate Git administrative directory", async (t) => {
  ...
  const result = await run([
    "capture-source", "project:source.txt", "--to", path.join(admin, "engram-output"),
    "--project-root", root, "--json",
  ]);
...
test("M2b output paths cannot overwrite sources, bundle files, symlinks, or existing files", async (t) => {
  ...
  for (const output of [
    ...
    const result = await run([
      "capture-source", "project:docs/guide ü.md", "--to", output,
      "--project-root", root, "--json",
    ]);
