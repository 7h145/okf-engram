...
async function killPausedMutation(root, id, file, pauseAt, ifMatch) {
  const child = fork(pausedMutation, [root, id, file, pauseAt, ifMatch ?? ""], {
...
test("R8 a stale lock left by a killed process is recoverable", async (t) => {
  ...
  const value = await withBundleLock(bundle, async () => "recovered", {
    stale: 2_000, update: 1_000, retries: 40,
  });
...
test("R8 simultaneous independent concept writes are both retained", async (t) => {
  ...
  const results = await Promise.all([
    run(["put", "independent/a", "--from", a, "--project-root", root, "--json"]),
    run(["put", "independent/b", "--from", b, "--project-root", root, "--json"]),
  ]);
  assert.deepEqual(results.map((result) => result.code), [0, 0]);
  for (const id of ["independent/a", "independent/b"]) {
    assert.equal((await run(["get", id, "--project-root", root])).code, 0);
  }
});
...
test("R8 kill before replacement retains old-complete concept", async (t) => {
  ...
  let result = await run(["put", "replace", "--from", oldFile, "--project-root", root, "--json"]);
  ...
  result = await run(["get", "replace", "--project-root", root, "--json"]);
...
test("R8 kill after replacement leaves new-complete concept and repairable index", async (t) => {
  ...
  await fs.rm(`${bundle}.lock`, { recursive: true, force: true });
  let result = await run(["get", "after-kill", "--project-root", root, "--json"]);
  assert.equal(result.code, 0, result.stderr);
